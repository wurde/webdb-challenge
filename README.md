...
RDBMS is short for Relational Database Management System and SQL is short for
Structured Query Language. An RDBMS is an implementation of a database while
SQL is a language implemented inside a database.

1. Why do tables need a `primary key`?

We need a way to identify a record. To distinguish one record from another we
need a column or a set of columns that are unique for each record. The primary
key in one table can be used as a foreign key in other tables.

1. What is the name given to a table column that references the primary key on another table.

A foreign key. This is a key referencing a primary key in another table.

1. What do we need in order to have a _many to many_ relationship between two tables.

We need a third table. This third table will have foreign keys for both tables
that comprise the many to many relationship. I call this a join table, but it
could also be called a bridge table. Having a name for something is *extremely
useful* when trying to communicate, discuss, or reason about that thing.
